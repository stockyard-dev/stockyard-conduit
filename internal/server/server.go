package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-conduit/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-conduit"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/syncs",s.handleList)
    s.mux.HandleFunc("POST /api/syncs",s.handleCreate)
    s.mux.HandleFunc("DELETE /api/syncs/{id}",s.handleDelete)
    s.mux.HandleFunc("POST /api/syncs/{id}/run",s.handleRun)
